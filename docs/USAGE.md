...
- **🔄 更新（重新抓取）**：标题栏的「🔄 更新」按钮可触发对当前所选来源的重新抓取任务
  - 多源书籍会通过顶部「切换源」按钮切换当前来源，更新只针对当前选中的源
  - 采用增量抓取：已下载的图片会跳过，只补抓缺失章节/图片，已完成的章节直接复用
  - 发起时会自动停止该资源同一来源下正在运行的旧任务（旧任务标记为 cancelled）
  - 按钮进入加载态并显示进度（如 `抓取中... 12/50`），每 2 秒轮询任务状态；完成后自动刷新详情页
- **导出 PDF**：
