...
export class WebtoonsParser {
  ...
  parseDetail(html: string): ComicDetail {
    ...
    const dayInfoHasCompleted =
      $('p.day_info').find('.txt_ico_completed').length > 0 ||
      $('p.day_info').text().includes('完結');
    const dayInfoHasUp =
      $('p.day_info').find('.txt_ico_up').length > 0 ||
      $('p.day_info').text().includes('更新');

    const isCompleted =
      dayInfoHasCompleted || /completed|完結/i.test(detailInfoText);
