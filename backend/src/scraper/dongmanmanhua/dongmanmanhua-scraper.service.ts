...
import { DongmanmanhuaParser } from './dongmanmanhua-parser';
...
@Injectable()
export class DongmanmanhuaScraperService extends BaseComicScraper {
  protected readonly logger = new Logger(DongmanmanhuaScraperService.name);
  private readonly parser: DongmanmanhuaParser;

  protected get baseUrl(): string {
    return 'https://www.dongmanmanhua.cn';
  }
  ...
  constructor(
    ...
    this.parser = new DongmanmanhuaParser();
  }
  ...
  async discoverCatalog(
    ...
    // dongmanmanhua.cn 的 /genre 页一次性返回全部作品(前端按 tab 过滤)
    let discovered = 0;
    let newCount = 0;
    const seenTitleNos = new Set<number>();

    if (taskId) this.checkCancelled(taskId);
    const genreUrl = `${this.baseUrl}/genre`;
    this.logger.log('抓取分类目录: /genre');
    if (taskId)
      await this.taskService.log(taskId, 'info', '抓取分类目录: /genre');

    const { html } = await this.fetchPage(genreUrl);
    const allCards = this.parser.parseGenreCards(html);
    ...
    for (const card of allCards) {
      ...
      if (!existing) {
        ...
        if (!resource) {
          const genreName = this.genreSlugToName(card.genre);
        ...
        const genreName2 = this.genreSlugToName(card.genre);
        if (genreName2) {
  ...
  private genreSlugToName(slug: string): string {
  ...
  public async ensureSourceSite(): Promise<SourceSite> {
    let site = await this.sourceSiteRepo.findOne({
      where: { domain: 'www.dongmanmanhua.cn' },
    });
    if (!site) {
      site = this.sourceSiteRepo.create({
        ...
        ageRating: this.ageRating,
        config: { baseUrl: this.baseUrl, genresPath: '/genre' },
  ...
  private async findListUrl(
    ...
    const { html } = await this.fetchPage(`${this.baseUrl}/genre`);
  ...
  private async saveResource(
    detail: ReturnType<DongmanmanhuaParser['parseDetail']>,
  ): Promise<Resource> {
    let resource = await this.resourceRepo.findOne({
  ...
  private async saveChapter(
    resource: Resource,
    site: SourceSite,
    ep: ReturnType<DongmanmanhuaParser['parseEpisodeList']>[0],
