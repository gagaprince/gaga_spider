...
import { AcgnParser } from './acgn-parser';
import {
  SourceSite,
  SiteResourceType,
...
const CATEGORY_IDS = Array.from({ length: 33 }, (_, i) => i + 1);

@Injectable()
export class AcgnScraperService extends BaseComicScraper {
  protected readonly logger = new Logger(AcgnScraperService.name);
  private readonly parser: AcgnParser;

  protected get baseUrl(): string {
    return 'https://comic.acgn.cc';
  }
  ...
  private async doScrape(
    ...
    const detailUrl = `${this.baseUrl}/manhua-${slug}.htm`;
    ...
    if (detailHtml.includes('資料已刪除')) {
      throw new Error(`漫画已被删除: ${slug}`);
    }
  ...
  async discoverCatalog(
    ...
    this.logger.log('开始抓取 ACGN.cc 目录...');
    const site = await this.ensureSourceSite();
    ...
    for (const catId of CATEGORY_IDS) {
      ...
      const firstUrl = `${this.baseUrl}/cate-${catId}.htm`;
      this.logger.log(`分类 ${catId}: ${firstUrl}`);

      let firstHtml: string;
      try {
        const result = await this.fetchPage(firstUrl);
        firstHtml = result.html;
      } catch (e) {
        this.logger.warn(`分类 ${catId} 首页抓取失败,跳过: ${e}`);
        continue;
      }

      const totalPages = this.parser.parseLastPage(firstHtml);
      this.logger.log(`分类 ${catId}: 共 ${totalPages} 页`);
      ...
      for (let page = 2; page <= totalPages; page++) {
        if (taskId) this.checkCancelled(taskId);
        const pageUrl = `${this.baseUrl}/cate-${catId}.htm?page=${page}`;
        if (taskId)
          await this.taskService.log(
            taskId,
            'info',
            `分类 ${catId} 第 ${page}/${totalPages} 页`,
          );
        ...
        try {
          ...
          this.logger.warn(`分类 ${catId} 第 ${page} 页抓取失败,跳过: ${e}`);
        }
      }
    }
  ...
  private async processCard(
    card: ReturnType<AcgnParser['parseComicCards']>[0],
    site: SourceSite,
    ...
    const coverUrl = this.resolveUrl(card.coverUrl);
    ...
    if (!existing) {
      ...
      if (!resource) {
        ...
        if (localCover) {
          ...
          if (!resource.coverUrl) resource.coverUrl = coverUrl;
          await this.resourceRepo.save(resource);
        }
      }
      ...
      if (existing.resourceId) {
        ...
        if (res && !this.coverFileExists(res.localCoverPath) && coverUrl) {
          const localCover = await this.downloadCover(
            card.slug,
            coverUrl,
            'other',
          ...
          if (localCover) {
            ...
            if (!res.coverUrl) res.coverUrl = coverUrl;
  ...
  private async saveResource(
    detail: ReturnType<AcgnParser['parseDetail']>,
    detailUrl: string,
  ): Promise<Resource> {
    let resource = await this.resourceRepo.findOne({
    ...
    const coverUrl = this.resolveUrl(detail.coverUrl);
    if (!resource) {
      const localCover = await this.downloadCover(
        detailUrl.split('/').pop()!.replace('.htm', '').replace('manhua-', ''),
        coverUrl,
      ...
      if (!this.coverFileExists(resource.localCoverPath) && coverUrl) {
        const localCover = await this.downloadCover(
          detailUrl.split('/').pop()!.replace('.htm', '').replace('manhua-', ''),
          coverUrl,
          detail.genres[0] || 'other',
  ...
  private async saveChapter(
    ...
    site: SourceSite,
    orderIndex: number,
    ch: ReturnType<AcgnParser['parseChapterList']>[0],
